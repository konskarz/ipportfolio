...
$title = $params->get('show_title') ? $this->escape($this->item->title) : false;
$style = $images->image_fulltext_alt ? $images->image_fulltext_alt : '';

if (!empty($this->item->pagination)
	&& $this->item->pagination
	&& !$this->item->paginationposition
	&& $this->item->paginationrelative) : 
		$app->input->set('ippager', $this->item->pagination);
endif;

if(isset($images->image_fulltext)
 && !empty($images->image_fulltext)) :
	$image_fulltext = htmlspecialchars($images->image_fulltext);
	if(strpos($image_fulltext, 'vimeo.com') !== false) :
		$this->document->addScript('//f.vimeocdn.com/js/froogaloop2.min.js');
		$vimeoid = 'p' . hash('md5', $image_fulltext);
	else :
		$style .= 'background-image:url(' . $image_fulltext . ');';
	endif;
endif;

if(isset($image_fulltext) || $title) : // banner image, video, title ?>
	<div id="banner" class="banner"<?php
		echo $style !== '' ? ' style="' . $style . '"' : ''; ?>><?php
	if(isset($vimeoid)) : // video ?>
		<iframe id ="<?php echo $vimeoid; ?>" src="<?php
			echo $image_fulltext;
			?>?title=0&byline=0&portrait=0&color=f3f3f4&api=1&player_id=<?php
			echo $vimeoid; // use  frameborder="0" for IE7 ?>">
		</iframe><?php
	endif; // video end
	if($title) : // title ?>
		<div class="banner-caption"><div class="container">
...
<div class="item-page<?php echo $this->pageclass_sfx?> container">
	<?php if ($canEdit && $useDefList && ($info == 0 || $info == 2)) : ?>
